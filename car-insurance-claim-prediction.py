...
Original file is located at
    https://colab.research.google.com/drive/1bf9-5EEcjNHsIsr6aNpxJF_DdDRjcZ_6
"""
...
# CatBoost
joblib.dump(cat_baseline, os.path.join(MODEL_DIR, "catboost_baseline.pkl"))
joblib.dump(cat_weighted, os.path.join(MODEL_DIR, "catboost_weighted.pkl"))
joblib.dump(rand_search_cat.best_estimator_, os.path.join(MODEL_DIR, "catboost_tuned.pkl"))
joblib.dump(rand_search_cat_weighted.best_estimator_, os.path.join(MODEL_DIR, "catboost_weighted_tuned.pkl"))

# Random Forest (example)
joblib.dump(rand_search_rf.best_estimator_, os.path.join(MODEL_DIR, "rf_tuned.pkl"))
joblib.dump(rand_search_rf_smote.best_estimator_, os.path.join(MODEL_DIR, "rf_smote_tuned.pkl"))

# Logistic Regression (example)
joblib.dump(rand_search_log.best_estimator_, os.path.join(MODEL_DIR, "logreg_tuned.pkl"))
joblib.dump(rand_search_log_weighted.best_estimator_, os.path.join(MODEL_DIR, "logreg_weighted_tuned.pkl"))

print("✅ All models saved successfully in:", MODEL_DIR)

# CatBoost results
df_cat_results.to_csv(os.path.join(RESULT_DIR, "catboost_results.csv"), index=False)

# Random Forest results
df_rf_results.to_csv(os.path.join(RESULT_DIR, "randomforest_results.csv"), index=False)

# Logistic Regression results
df_log_results.to_csv(os.path.join(RESULT_DIR, "logreg_results.csv"), index=False)

print("✅ All evaluation results saved successfully in:", RESULT_DIR)
...
display(leaderboard.sort_values("Rank (by F1 Test)") if "Rank (by F1 Test)" in leaderboard.columns else leaderboard)
...
"""## Comprehensive Report: Car Insurance Claim Prediction

### 1. Data Loading and Initial Exploratory Data Analysis (EDA)

The analysis began by loading the `Car_Insurance_Claim.csv` dataset. The dataset contains information about car insurance policyholders and whether they filed a claim (`OUTCOME`).

**Dataset Shape:** The dataset contains **10000 rows** and **19 columns**.

**Data Types:** The dataset contains a mix of numerical and categorical features.

**Missing Values:**
- `CREDIT_SCORE`: 982 missing values
- `ANNUAL_MILEAGE`: 957 missing values
All other columns have no missing values.

**Target Distribution:** The target variable `OUTCOME` is imbalanced:
- `0.0` (No Claim): 6867 (68.67%)
- `1.0` (Claim): 3133 (31.33%)

**Summary of Numeric Features:**
The numeric features include `ID`, `CREDIT_SCORE`, `VEHICLE_OWNERSHIP`, `MARRIED`, `CHILDREN`, `POSTAL_CODE`, `ANNUAL_MILEAGE`, `SPEEDING_VIOLATIONS`, `DUIS`, `PAST_ACCIDENTS`, and `OUTCOME`. Descriptive statistics provided insights into their central tendency, spread, and range.

**Summary of Categorical Features:**
The categorical features are `AGE`, `GENDER`, `RACE`, `DRIVING_EXPERIENCE`, `EDUCATION`, `INCOME`, `VEHICLE_YEAR`, and `VEHICLE_TYPE`. The summaries showed the unique categories and their frequencies for each feature. Notable observations include:
- `RACE`: Heavily skewed towards 'majority'.
- `VEHICLE_TYPE`: Heavily skewed towards 'sedan'.
- `POSTAL_CODE`: Has a high number of unique values, which might not be useful as a direct feature.

Initial visualizations (histograms, boxplots, countplots) were generated to further explore the distributions of individual features and their relationship with the target variable. These plots are saved in the `plots/eda` directory.
"""
...
for col in categorical_cols:
    plt.figure(figsize=(6,4))
    sns.countplot(x=col, hue=TARGET_COL, data=df_raw)
    plt.title(f"{col} vs {TARGET_COL}")
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, f"count_{col}_vs_{TARGET_COL}.png"))
    plt.show()

for col in categorical_cols:
    plt.figure(figsize=(6,4))
    num_categories = min(10, df_raw[col].nunique())  # use actual number of categories if less than 10
    top_categories = df_raw[col].value_counts().nlargest(num_categories)

    sns.barplot(x=top_categories.index, y=top_categories.values)
    plt.title(f"Top {num_categories} Categories in {col}")
    plt.xlabel(col)
    plt.ylabel("Count")
    plt.xticks(rotation=0)  # Upright labels
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, f"top_{col}.png"))
    plt.show()

# --------------------------------
# Block 1: Categorical Features vs Target (Proportion)
# --------------------------------
for col in categorical_cols:
    plt.figure(figsize=(8,5))

    # Grouped stacked bar chart for target proportion per category
    ctab = pd.crosstab(df_raw[col], df_raw[TARGET_COL], normalize='index')
    ctab.plot(kind='bar', stacked=True, figsize=(8,5), rot=0)
    plt.title(f"{col} vs {TARGET_COL} (Proportion)")
    plt.ylabel("Proportion")
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/{col}_vs_{TARGET_COL}_proportion.png")
    plt.show()

# --------------------------------
# Block 2: Correlations Heatmap
# --------------------------------
plt.figure(figsize=(8,6))
sns.heatmap(df_raw[numeric_cols].corr(), annot=True, cmap='coolwarm', fmt=".2f")
plt.title("Correlation Heatmap of Numeric Features")
plt.savefig(f"{PLOT_DIR}/numeric_corr_heatmap.png")
plt.show()

# --------------------------------
# Block 4: Numeric Feature Distributions & Skew
# --------------------------------
# What: Visualize numeric columns to see their distribution and detect skewness.
# Why: Helps decide if transformation (e.g., log, sqrt) is needed.
# Layman: Like checking if ingredients are lumpy or uneven before cooking.

for col in numeric_cols:
    plt.figure(figsize=(8,5))
    sns.histplot(df_raw[col], kde=True, bins=30, color='skyblue')
    plt.title(f"Distribution & Skew of {col}")
    plt.xlabel(col)
    plt.ylabel("Count")
    plt.tight_layout()
...
# --------------------------------
# Block 5: Boxplots for Outlier Detection
# --------------------------------
# What: Identify potential outliers in numeric features using boxplots.
# Why: Detect extreme values that could distort model training.
# Layman: Like spotting rotten pieces among fruits before making a salad.

for col in numeric_cols:
    plt.figure(figsize=(8,5))
    sns.boxplot(x=df_raw[col], color='lightgreen')
    plt.title(f"Boxplot of {col} for Outlier Detection")
    plt.xlabel(col)
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/{col}_boxplot.png")
    plt.show()
...
# =========================================
# Gradio Interface for CatBoost Car Insurance Claim Prediction
# =========================================
...
leaderboard_metrics = {
    ...
    "F1 Test": cat_metrics.get("f1_test", 0),
    "ROC-AUC Test": cat_metrics.get("roc_auc_test", 0),
...
# ---------------------------
numeric_features = ['credit_score', 'annual_mileage', 'risk_score']
cat_features = ['age', 'gender', 'driving_experience', 'education',
...
def predict_claim(age, gender, driving_experience, education, income,
                  credit_score, vehicle_year, vehicle_type, married, children,
                  annual_mileage, risk_score):
    ...
    expected_cols = numeric_features + cat_features + ['married', 'children']
